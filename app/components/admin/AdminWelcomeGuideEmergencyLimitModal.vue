...
const limitDesc = computed(() =>
  adminUiFormat(ui.value.welcomeGuide.emergency.limitDesc, { max: WELCOME_GUIDE_MAX_EMERGENCY_COUNT })
)
    ...
    <Transition name="hostiv-modal-fade">
      <div v-if="open" class="hostiv-modal hostiv-modal--delete" data-backdrop="true" role="presentation" @click="onBackdropClick" @keydown="onKeydown">
        <Transition name="hostiv-modal-panel" appear>
          <div class="hostiv-modal__panel hostiv-modal__panel--delete" role="alertdialog" aria-modal="true" aria-labelledby="admin-welcome-guide-emergency-limit-title" aria-describedby="admin-welcome-guide-emergency-limit-desc" @click.stop>
            ...
            <header class="hostiv-modal__head hostiv-modal__head--delete">
              ...
              <div class="hostiv-modal__head-text">
                <h2 id="admin-welcome-guide-emergency-limit-title" class="hostiv-modal__title">{{ ui.welcomeGuide.emergency.limitTitle }}</h2>
                <p id="admin-welcome-guide-emergency-limit-desc" class="hostiv-modal__subtitle">{{ limitDesc }}</p>
              </div>
            </header>
            <footer class="hostiv-modal__danger-footer">
              <button type="button" class="hostiv-btn hostiv-btn--primary" @click="emit('close')">{{ ui.common.understood }}</button>
            </footer>
          </div>
        </Transition>
      </div>
